...
The training pipeline also uses CAMELS static attribute tables from the same data folder. By default it loads 39 numeric catchment attributes from `camels_topo.txt`, `camels_clim.txt`, `camels_soil.txt`, `camels_vege.txt`, and `camels_geol.txt`; `camels_hydro.txt` is intentionally excluded to avoid feeding streamflow-derived hydrologic signatures into the runoff predictor.
...
```text
Input:  forcing variables from the previous 90 days for one basin
...
In addition to these time-varying/windowed inputs, each basin has a static context vector:

```python
static_features.shape = (num_basins, 39)
```

The static vector is selected by `basin_slot` for each sequence window and passed to the model separately from the daily forcing sequence.

So with the current 90-day lookback, the transformer input shape for one batch might be:

```python
batch_X.shape = (batch_size, 90, 10)
batch_static.shape = (batch_size, 39)
```
...
```python
features.shape = (num_basin_days, num_features)
targets.shape = (num_basin_days,)
window_start.shape = (num_windows,)
static_features.shape = (num_basins, 39)
```
