...
def main() -> None:
    ...
    for split_name in args.splits:
        ...
        if static_columns and len(relationship_rows) > args.n_static_attribute_plots:
            plot_static_attribute_relationships(
                output_base.with_name(f"{output_base.name}_nse_vs_all_static_attributes.png"),
                scores,
                static_values,
                static_columns,
                relationship_rows,
                title=f"{model_label} {split_name.upper()} Per-Basin NSE vs All Static Attributes",
                scale_label=static_scale_label,
                top_n=len(relationship_rows),
                n_cols=5,
            )
